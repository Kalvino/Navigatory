...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'Route'
        db.create_table('oxero_route', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
        ))
        db.send_create_signal('oxero', ['Route'])

        # Adding M2M table for field segments on 'Route'
        db.create_table('oxero_route_segments', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('route', models.ForeignKey(orm['oxero.route'], null=False)),
            ('segment', models.ForeignKey(orm['oxero.segment'], null=False))
        ))
        db.create_unique('oxero_route_segments', ['route_id', 'segment_id'])

        # Adding model 'RecommendedItinerary'
        db.create_table('oxero_recommendeditinerary', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
        ))
        db.send_create_signal('oxero', ['RecommendedItinerary'])

        # Adding M2M table for field routes on 'RecommendedItinerary'
        db.create_table('oxero_recommendeditinerary_routes', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('recommendeditinerary', models.ForeignKey(orm['oxero.recommendeditinerary'], null=False)),
            ('route', models.ForeignKey(orm['oxero.route'], null=False))
        ))
        db.create_unique('oxero_recommendeditinerary_routes', ['recommendeditinerary_id', 'route_id'])

        # Deleting field 'Segment.route'
        db.delete_column('oxero_segment', 'route')


    def backwards(self, orm):
        # Deleting model 'Route'
        db.delete_table('oxero_route')

        # Removing M2M table for field segments on 'Route'
        db.delete_table('oxero_route_segments')

        # Deleting model 'RecommendedItinerary'
        db.delete_table('oxero_recommendeditinerary')

        # Removing M2M table for field routes on 'RecommendedItinerary'
        db.delete_table('oxero_recommendeditinerary_routes')

        # Adding field 'Segment.route'
        db.add_column('oxero_segment', 'route',
